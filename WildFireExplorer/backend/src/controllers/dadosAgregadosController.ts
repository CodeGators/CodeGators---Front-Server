...
export const getRiscoAgregadoPorEstado = async (req: Request<{}, {}, {}, DadosAgregadosQueryParams>, res: Response): Promise<void> => {
  const { mes, estado, satelite } = req.query;
  ...
  params.push(mes);
  where.push(`to_char(fc.data_hora_gmt::date, 'YYYY-MM') = $${paramIndex}`);
  paramIndex++;

  if (estado && !isNaN(Number(estado))) {
    params.push(Number(estado));
    where.push(`fc.estado_id = $${paramIndex}`);
    paramIndex++;
  }

  if (satelite) {
    params.push(satelite);
    where.push(`fc.satelite = $${paramIndex}`);
    paramIndex++;
  }

  // >>> FILTROS ROBUSTOS PARA risco_fogo, COM CASTING NA WHERE CLAUSE
  where.push(`fc.risco_fogo IS NOT NULL`);
  // NOVO: Converta para NUMERIC antes da comparação
  where.push(`fc.risco_fogo::NUMERIC <> -999.00`); // Casting aqui
  where.push(`fc.risco_fogo::NUMERIC >= 0.01`);    // Casting aqui
  where.push(`fc.risco_fogo::NUMERIC <= 1.00`);    // Casting aqui


  const sql = `
    SELECT
      e.nm_uf AS estado,
      AVG(fc.risco_fogo::NUMERIC) AS risco_medio -- Manteve o casting para AVG
    FROM focos_calor fc
    JOIN estados e ON fc.estado_id = e.cd_uf
    WHERE ${where.join(' AND ')}
    GROUP BY e.nm_uf
...
export const getRiscoAgregadoPorBioma = async (req: Request<{}, {}, {}, DadosAgregadosQueryParams>, res: Response): Promise<void> => {
  ...
  if (bioma && !isNaN(Number(bioma))) {
    params.push(Number(bioma));
    where.push(`fc.bioma_id = $${paramIndex}`);
    paramIndex++;
  }
