...
export default function ChartPage() {
  ...
  useEffect(() => {
    // Agora, os gráficos de focos/áreas/risco só fazem sentido se o tipo for 'Focos', 'Queimadas' ou 'Risco'
    ...
    // Satélite é relevante para 'Focos' e 'Risco' (já que ambos vêm da tabela focos_calor)
    if ((filters.tipo === 'Focos' || filters.tipo === 'Risco') && filters.satelite) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        } else if (filters.tipo === 'Risco') { // NOVO: Rota para risco de fogo
          stateApiUrl = `http://localhost:4000/api/dados/risco-por-estado?${params}`;
          biomaApiUrl = `http://localhost:4000/api/dados/risco-por-bioma?${params}`;
        }
        ...
        if (filters.tipo === 'Focos') {
            ...
            sortKey = "risco_medio"; // NOVO: Ordenar por risco_medio
            mapDataFunction = (item: any) => ({
                estado: item.estado,
                bioma: item.bioma,
                risco_medio: parseFloat(item.risco_medio), // NOVO: Parsear para float
            });
        }
